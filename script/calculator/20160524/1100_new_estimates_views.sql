...
select
  'ME',
  'ME'||survey_modeled_extrapolations.id input_zone_id,
  population_submission_id,
  site_name,
  site_name,
  area,
  ...
  actually_seen,
  survey_geometry_id
from survey_modeled_extrapolations
  join population_submissions on population_submissions.id=population_submission_id
union
select
  'O',
...
select
  ...
  CASE
    ...
    WHEN estimate_type='ME' THEN
      CASE WHEN quality_level = 1 THEN 'G' ELSE 'E' END
