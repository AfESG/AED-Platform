...
CREATE OR REPLACE VIEW estimate_factors_analyses_categorized_totals_country_for_add AS
  SELECT
    ...
    1.96*sqrt(sum(lower_bound_variance) FILTER (WHERE "CATEGORY" IN ('J', 'K', 'L'))) as "LOWER_CONFIDENCE",
    1.96*sqrt(sum(upper_bound_variance) FILTER (WHERE "CATEGORY" IN ('J', 'K', 'L'))) as "UPPER_CONFIDENCE"
